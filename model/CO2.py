...
class CO2Node:                                                      # defining the CO2 Node
    ...
    @property
    def label_proba(self):
        if not hasattr(self, '_label'):
            classes, counts = np.unique(self.labels, return_counts=True)
            self._label_proba = [0, 0, 0]
            for i in range(len(classes)):
                self._label_proba[int(classes[i])] = counts[i]/sum(counts)
        return self._label_proba
...
class ContinuouslyOptimizedObliqueTree(BaseEstimator):
            ...
            while not converged:
                ...
                for t in range(1, (self.tau + 1)):
                    ...
                    for i in range(len(sample)):
                        ...
                        if boundL >= boundR:
                            w = w + (self.eta / np.sqrt(t)) * (1 + s) * x_sample[i, :]
                        ...
                        if np.linalg.norm(w) ** 2 > self.nu:
                            w = w * np.sqrt(self.nu) / np.linalg.norm(w)
    ...
    def predict_proba(self, X):
        ...
        predictions = []
        for i in range(size):
            predictions.append(predict_single(X[i, :]))
