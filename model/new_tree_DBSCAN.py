...
class tree_stump(object):
    ...
    def fit(self, data, seed):
        ...
        if len(list(set(data.iloc[:, -1].astype("int")))) == 1:
            ...
            self.tree, self.feature, self.train_data, self.best_eps = self.build_tree_stump(data, [seed])
    ...
    def build_tree_stump(self, dataset, seed):
        ...
        for z in range(dataset.shape[1]-1):
            ...
            for j in [i for i in range(dataset.shape[1]-1) if i not in seed]:
                for eps in [0.3, 0.4, 0.5, 0.6, 0.7]:
                    try:
                        clf = DBSCAN(eps=eps, min_samples=10).fit(dataset.iloc[:, seed+[j]])
        ...
        return final_root, seed, np.array(dataset.iloc[:, seed]), best_eps
    ...
    def predict(self, x):
        ...
        if len(self.tree) == 2:
            ...
            x = np.array(x)
            clf = DBSCAN(eps=self.best_eps, min_samples=10).fit(np.vstack((x[:, self.feature], self.train_data)))
