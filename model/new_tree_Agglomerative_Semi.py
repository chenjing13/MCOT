...
class tree_stump(object):
    ...
    def fit(self, data, seed):
        ...
        label = list(set(data.iloc[:, -1].astype("int")))
        if len(label) == 2 and -1 in label:
            label.remove(-1)
            self.tree = {"score": label[0], "c_label": []}
        elif len(label) == 1 and -1 not in label:
            self.tree = {"score": label[0], "c_label": []}
        elif len(label) == 1 and -1 in label:
            self.tree = {"score": -1, "c_label": []}
        else:
    ...
    def get_split(self, c, data):
        ...
        for group in groups:
            ...
            groups_remove.append([row for row in group if row[-1] != -1])
            if len(dict(Counter([row[-1] for row in group if row[-1] != -1]))) != 0:
                groups_value.append(max(dict(Counter([row[-1] for row in group if row[-1] != -1])),
                                    key=dict(Counter([row[-1] for row in group if row[-1] != -1])).get))
            else:
                groups_value.append(-1)
        gini = self.gini_index(groups_remove, groups_value)
...
class Decision_trees(BaseEstimator):
    ...
    def fit(self,  x, y):
        ...
        unkunow_data = x.loc[(x['y'] == -1)]
        kunow_data = x.loc[(x['y'] != -1)]
        self.classes_ = np.array(sorted(list(dict(Counter(kunow_data["y"])).keys())))
        self.min_samples_leaf = self.n_samples * self.min_samples_leaf
        self.tree = self.build_tree(kunow_data, unkunow_data, self.max_depth, self.min_samples_leaf)
        print(self.tree)
    ...
    def build_tree(self, data, unknow_x, max_depth, min_size):  # Build a decision tree
        ...
        if self.method == "SMOTE":
            ...
            try:
                ...
                x_resampled, y_resampled = SMOTE(random_state=0).fit_resample(np.vstack((data[:, :-1], data[:, :-1])), list(map(int, np.hstack((data[:, -1], data[:, -1])))))
        ...
        clf_0, clf_1, clf_2 = self.ova_change(data, unknow_x)
        root = self.get_split(data, clf_0, clf_1, clf_2)
        self.split(root, max_depth, min_size, 1)

        return root

    def ova_change(self, data, unknow_x):

        data_0 = data.copy()
        data_0.loc[data_0.iloc[:, -1] == 2] = 1
        data_0 = pd.concat([data_0, unknow_x])
        data_1 = data.copy()
        data_1.loc[data_1.iloc[:, -1] == 2] = 0
        data_1 = pd.concat([data_1, unknow_x])
        data_2 = data.copy()
        data_2.loc[data_2.iloc[:, -1] == 1] = 0
        data_2 = pd.concat([data_2, unknow_x])
    ...
    def to_terminal(group):  # Create a terminal node value
        ...
        outcomes = [row[-1] for row in group if row[-1] != -1]
        if len(outcomes) == 0:
            return 2
        else:
    ...
    def split(self, node, max_depth, min_size, depth):
        ...
        if len(groups[0]) + len(groups[1]) + len(groups[2]) < 5:
            ...
            clf_0, clf_1, clf_2 = self.ova_change(data[data.iloc[:, -1] != -1], data[data.iloc[:, -1] == -1])
