...
class node(object):													             # A Node definition
	...
	def find_best_split(self, max_features='sqrt'):
		...
		def func_gradient(a):
			...
			w_R_eachClass = np.array([sum(w_R[Y == k]) for k in range(class_num)])
			w_L_eachClass = np.array([sum(w_L[Y == k]) for k in range(class_num)])
			la = np.log2(w_L_eachClass[Y.astype("int")] * w_R.sum() + epsilonepsilon) - np.log2(w_R_eachClass[Y.astype("int")] * w_L.sum() + epsilonepsilon)
			beta = la * p * (1 - p)
...
class BaseObliqueTree(BaseEstimator):
    ...
    def fit(self, X, Y):
        self.X = X
        self.Y = Y
        self.classNum = int(self.Y.max() + 1)
    ...
    def predict_forAllInstance(self, x):
        present_node = self.root_node
        while not (present_node.is_leaf):
            if present_node.split.test_forOneInstance(x):
                present_node = present_node.LChild
                ...
                present_node = present_node.RChild
        return present_node.class_distribution
    ...
    def predict_proba(self, X):
        ...
        for i in range(m):
            x = X[i]
            Y_predicted.append(self.predict_forAllInstance(x))
...
def sigmoid(z):
    ...
    return 1 / (np.exp(- z.astype(np.float)) + 1)
